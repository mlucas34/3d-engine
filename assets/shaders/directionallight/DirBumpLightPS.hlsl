...
float4 psmain(PS_INPUT input) : SV_TARGET
{
    ...
    float dotNL = dot(m_lightDir.xyz, input.tbn[2]);
    ...
    float diffuseAmount = max(0.0, dot(m_lightDir.xyz, normal.xyz));
    ...
    float3 reflectedLight = reflect(m_lightDir.xyz, normal.xyz);
    float shininess = 10.0f;
    float specularAmount = 0;
    
    if (dotNL > 0)
    {
        pow(max(0.0f, dot(reflectedLight, dirToCam)), shininess);
    } 
